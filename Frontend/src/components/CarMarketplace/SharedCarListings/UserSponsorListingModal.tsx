...
const UserSponsorListingModal: React.FC<Props> = ({ listing, walletBalance, onClose, onSuccess, showToast, apiPrefix }) => {
    ...
    const calculatePrice = async () => {
        setCalculating(true);
        try {
            // Using api helper instead of fetch for consistency and interceptors
            const response = await api.get(`${apiPrefix}/listings/sponsor-price?days=${duration}`);
            const data = response.data;
            setPrice(data.price);
            setTier(data.tier);
        } catch (error) {
            console.error('Failed to calculate price:', error);
            // Fallback or error handling - might not exist for users yet
    ...
    const handleSponsor = async () => {
        ...
        setLoading(true);
        try {
            const response = await api.post(`${apiPrefix}/listings/${listing.id}/sponsor`, {
            ...
            if (response.data.success) {
                ...
                showToast(response.data.error || 'فشل تفعيل الرعاية', 'error');
            }
        } catch (error: any) {
            showToast(error.response?.data?.message || 'حدث خطأ أثناء تفعيل الرعاية', 'error');
        } finally {
            setLoading(false);
        }
    };
...
export default UserSponsorListingModal;
