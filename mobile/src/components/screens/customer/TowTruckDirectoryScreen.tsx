...
export default function TowTruckDirectoryScreen() {
    ...
    const loadTowTrucks = async () => {
        ...
        try {
            ...
            const data: any = await DirectoryService.getTowTrucks(params);
            setTowTrucks(Array.isArray(data) ? data : data.data || []);
        } catch (error) {
    ...
    const renderTowTruckCard = ({ item }: { item: TowTruck }) => {
        const coverImage = item.gallery && item.gallery.length > 0
            ? { uri: item.gallery[0].url || item.gallery[0].path }
            : null;

        return (
            <TouchableOpacity
                style={styles.card}
                onPress={() => router.push(`/(customer)/tow-trucks/${item.id}`)}
            >
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top']}>
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>دليل السطحات</Text>
            ...
            {loading ? (
                ...
                <FlatList
                    data={towTrucks}
                    renderItem={renderTowTruckCard}
                    keyExtractor={(item) => item.id.toString()}
                    contentContainerStyle={styles.listContent}
                    ListEmptyComponent={
                        <View style={styles.centerContainer}>
                            <Text style={styles.emptyText}>لا توجد سطحات متاحة حالياً</Text>
                        </View>
                    }
                />
            )}
